...
-- Single-row site settings (toggles for the public site, e.g. hiding the
-- tracker from About without touching any data). The boolean primary key
-- with a fixed default enforces there only ever being one row.
...
-- Anyone can read settings (the public About page needs to know whether
-- to render the tracker section at all).
...
-- Only signed-in admin users can change settings.
